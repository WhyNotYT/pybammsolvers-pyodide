...
def build_solvers():
    DEFAULT_INSTALL_DIR = os.environ.get(
        "PYBAMMSOLVERS_DEFAULT_LIB_DIR",
        str(pathlib.Path(__file__).parent.resolve() / ".idaklu"),
    )
    wasm_build = os.environ.get("EMSDK") is not None
    ...
    def install_sundials():
        ...
        cmake_cmd = ["emcmake", "cmake"] if wasm_build else ["cmake"]
        subprocess.run([*cmake_cmd, sundials_src, *cmake_args], cwd=build_dir, check=True)
    ...
    def check_libraries_installed():
        ...
        if platform.system() == "Linux":
            lib_suffix = ".a" if wasm_build else ".so"
            sundials_files = [file + lib_suffix for file in sundials_files]
        ...
        if platform.system() == "Linux":
            lib_suffix = ".a" if wasm_build else ".so"
            suitesparse_files = [file + lib_suffix for file in suitesparse_files]
        elif platform.system() == "Darwin":
